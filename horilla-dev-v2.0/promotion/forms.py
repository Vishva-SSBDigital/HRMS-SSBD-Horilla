...
#         label=_("Department"),
#         required=False,
#         widget=forms.Select(attrs={"class": "oh-input", "id": "id_department"}),  # no "oh-select" to avoid plugin issues
#         empty_label="---------",
#     )

#     # We’ll replace this full <select> via HTMX; keep a plain select here
#     proposed_designation = forms.ChoiceField(
#         label=_("Proposed Employee New Grade/Designation"),
#         required=True,
#         choices=[("", "---------")],
...
#         ]
#         widgets = {
#             "employee": forms.Select(attrs={"class": "oh-input", "id": "id_employee"}),
#             "current_status": forms.HiddenInput(),
#             "current_designation": forms.HiddenInput(),
#             "proposed_status": forms.Select(attrs={"class": "oh-input"}),
#             "proposed_designation": forms.Select(attrs={"class": "oh-input oh-select"}),
#             "proposed_basic_salary": forms.NumberInput(attrs={"class":"oh-input", "readonly": True, "step": "0.01"}),
#             "effective_from": forms.DateInput(attrs={"type":"date","class":"oh-input"}),
#             "current_cl": forms.HiddenInput(),
...
#         # HTMX: when employee changes, autofill; when department changes, load positions
#         self.fields["employee"].widget.attrs.update({
#             "id": "id_employee",
#             "class": "oh-input oh-select",
#             "hx-get": reverse("promotion:employee_lookup"),
...
#         self.fields["department"].widget.attrs.update({
#             "hx-get": reverse("promotion:positions_for_department"),
#             "hx-target": "#proposed-wrapper",  # replace the whole select
#             "hx-trigger": "change",
#             "hx-include": "#id_department",
#             "hx-swap": "innerHTML",
#         })
...
class PromotionForm(forms.ModelForm):
    ...
    # helper field (not stored)
    department = forms.ModelChoiceField(
        queryset=Department.objects.none(),
        label=_("Department"),
        required=False,
        widget=forms.Select(attrs={"class": "oh-input", "id": "id_department"}),
        empty_label="---------",
    )

    # must be a ModelChoiceField so posted id validates
    proposed_designation = forms.ModelChoiceField(
        label=_("Proposed Employee New Grade/Designation"),
        required=True,
        queryset=Designation.objects.all().order_by("name"),
        widget=forms.Select(attrs={"class": "oh-input", "id": "id_proposed_designation"}),
        empty_label="---------",
    )
    ...
    class Meta:
        ...
        fields = [
            "employee",
            "proposed_status",           # optional; we’ll default in view if missing
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Make proposed_status optional; we’ll default it in the view if omitted
        self.fields["proposed_status"].choices = [
            ("REGULAR", "Regular"),
            ("CONTRACT", "Contractual"),
        ]
        self.fields["proposed_status"].required = False
